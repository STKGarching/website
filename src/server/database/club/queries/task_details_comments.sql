...
FROM
    ...
    club.relationship r ON t.task_id = r.source_id
        LEFT JOIN
    club.comment c ON r.target_id = c.comment_id
        LEFT JOIN
    club.relationship r_comment ON c.comment_id = r_comment.source_id
        LEFT JOIN
    club.person p ON r_comment.target_id = p.person_id
        LEFT JOIN
